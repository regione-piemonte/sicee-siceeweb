...
public abstract class BaseAction<T extends BaseSessionAwareDTO> extends ActionSupport
	...
	static {
		...
		MenuStructure mnuGstDatiCertMenu = new MenuStructure("mnuGstDatiCert");

		MenuStructure mnuRegDatiCertMenu = new MenuStructure("mnuRegDatiCert");

		mnuGstDatiCertMenu.getSubmenus().add(mnuRegDatiCertMenu);
		MenuStructure mnuTransDatiCertMenu = new MenuStructure("mnuTransDatiCert");

		mnuGstDatiCertMenu.getSubmenus().add(mnuTransDatiCertMenu);
		MenuStructure mnuTransAnomaleDatiCertMenu = new MenuStructure("mnuTransAnomaleDatiCert");

		mnuGstDatiCertMenu.getSubmenus().add(mnuTransAnomaleDatiCertMenu);
		MenuStructure mnuInvioCurriculumMenu = new MenuStructure("mnuInvioCurriculum");

		mnuGstDatiCertMenu.getSubmenus().add(mnuInvioCurriculumMenu);

		menuMap.add(mnuGstDatiCertMenu);

		MenuStructure mnuGstCertMenu = new MenuStructure("mnuGstCert");

		MenuStructure mnuPrenotAttestatiMenu = new MenuStructure("mnuPrenotAttestati");

		mnuGstCertMenu.getSubmenus().add(mnuPrenotAttestatiMenu);
		MenuStructure mnuRicercaACEMenu = new MenuStructure("mnuRicercaACE");

		mnuGstCertMenu.getSubmenus().add(mnuRicercaACEMenu);
		MenuStructure mnuGestACEMenu = new MenuStructure("mnuGestACE");

		mnuGstCertMenu.getSubmenus().add(mnuGestACEMenu);
		MenuStructure mnuAnnullaACEMenu = new MenuStructure("mnuAnnullaACE");

		mnuGstCertMenu.getSubmenus().add(mnuAnnullaACEMenu);

		menuMap.add(mnuGstCertMenu);

		MenuStructure mnuMonitoraggioMenu = new MenuStructure("mnuMonitoraggio");

		MenuStructure vAmbienteMenu = new MenuStructure("vAmbiente");

		mnuMonitoraggioMenu.getSubmenus().add(vAmbienteMenu);

		menuMap.add(mnuMonitoraggioMenu);
	...
	private it.csi.sicee.siceeweb.business.BackEndFacade backEnd = null;

	public void setSpringBackEnd(it.csi.sicee.siceeweb.business.BackEndFacade be) {
		backEnd = be;
	}

	public it.csi.sicee.siceeweb.business.BackEndFacade getSpringBackEnd() {
		return backEnd;
	}

	private it.csi.sicee.siceeweb.business.SecurityHelper securityHelper = null;

	public void setSpringSecurityHelper(it.csi.sicee.siceeweb.business.SecurityHelper sh) {
		securityHelper = sh;
	}

	public it.csi.sicee.siceeweb.business.SecurityHelper getSpringSecurityHelper() {
