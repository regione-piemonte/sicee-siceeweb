...
public class StampaCopiaCertificatoAction extends BaseAction<BaseSessionAwareDTO> {
...
//	private int bufferSize;
	...
	public String execute() throws Exception {
		...
		LOG.debug("[StampaCopiaCertificatoAction::execute] BEGIN");
		try {
			...
			{
				certificato = getSoaIntegrationMgr().getStampaCertificato(numeroAttestato, "COPIA");
			}
			else
			{
			...
			ByteArrayInputStream filePdf = ext
					.stampaModulo(
							getCertificatoMgr().compilaCertificatoPerStampa(
									numeroAttestato),
							ServletActionContext.getServletContext(), "COPIA",
			...
			*/
			...
			LOG.debug("[StampaCopiaCertificatoAction::execute] END");
			if (inputStream != null) {
				inputStream.close();
			}
		}
